...
define(['socket.io', 'domain', 'user', 'util', 'jquery', 'jquery.event.gevent', 'jquery.event.ue'], function(io, Domain, User, util, $) {
  'use strict';
  ...
  login = function(){
	...
	socket.on('connect', function(){
	  if(!userId){
          ...
          newSocket = io.connect('/domains?token=' + token)
  ...
  connect = function(token, callback){
	var socket = io.connect('/domains?token=' + token);
	socket.on('connect', function(){
