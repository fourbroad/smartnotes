...
define(['im', 'util','jquery', 'util_b','css!../css/chat'], function(im, util, $, util_b) {
  'use strict';
  ...
  setPxSizes = function () {
    ...
    px_per_em = util_b.getEmSize( jqueryMap.$slider.get(0) );
  ...
  writeChat = function ( person_name, text, is_user ) {
    ...
    jqueryMap.$msg_log.append(
      '<div class="' + msg_class + '">'
      + util_b.encodeHtml(person_name) + ': '
      + util_b.encodeHtml(text) + '</div>'
    );
  ...
  onListchange = function(event) {
    var
      list_html = String(),
      people_db = im.getDB(),
      chatee    = im.get_chatee(),
      client = stateMap.client;

    people_db().each( function ( person, idx ) {
      var select_class = '', user = client.currentUser;

      if(person.name === user.id || person.cid =='a0') { return true;}
      ...
      list_html
        ...
        +  util_b.encodeHtml( person.name ) + '</div>';
    });
  ...
  onUpdatechat = function ( event, msg_map ) {
    var
      ...
      chatee    = configMap.im.get_chatee() || {},
      sender    = configMap.im.get_by_cid(sender_id);
    ...
    is_user = sender.id === stateMap.client.currentUser.id;
  ...
  onChangeClient = function (event, client) {
    im.leave();      
    stateMap.client = client;
    ...
    if(client.currentUser.isAnonymous()){
      ...
      im.init(client.token);   
      im.join(client.currentUser);
    }
  };
  //-------------------- END EVENT HANDLERS --------------------
  ...
  initModule = function($append_target, client) {
    var $list_box;
    ...
    $.gevent.subscribe($list_box, 'changeClient', onChangeClient);
