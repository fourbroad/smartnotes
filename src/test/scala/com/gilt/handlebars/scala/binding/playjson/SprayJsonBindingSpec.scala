...
class SprayJsonBindingSpec extends FunSpec with Matchers {
  def b(v: JsValue): SprayJsonBinding = new SprayJsonBinding(v)
  def b(v: String): SprayJsonBinding = b(v.parseJson)
  ...
  describe("isTruthy") {
    it("matches JavaScript truthy behavior") {
      ...
      b(JsObject(Seq():_*)).isTruthy should be(true)
      b(JsArray(Seq():_*)).isTruthy should be(true)
    }
  }
