...
class RootDomainAPISpec extends AsyncFlatSpec with Matchers {
  ...
  var helloCookies: immutable.Seq[HttpCookiePair] = _

  "Smartnotes" should "not allow to do any operations before the platform is initialized" in {
    apiClient.get("http://localhost:8080").map {
      case (code, _, jv) =>
        assert(code == StatusCodes.PreconditionRequired)
    }
  }

  it should "initialize platform with null content" in {
    apiClient.post("http://localhost:8080/_init").map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }

  it should "login in with administrator" in {
  ...
  it should "login in with new user" in {
    val user = """{"name":"administrator","password":"!QAZ)OKM"}"""
    //    val user = """{"name":"fourbroad","password":"z4bb4z"}"""
  ...
  it should "create a document set with null content" in {
    apiClient.post(uri = "http://localhost:8080/document_set", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
  ...
  it should "create a document set with content" in {
    apiClient.post(uri = "http://localhost:8080/document_set2", headers = List(Cookie(cookies)), entity = """{"name":"documentSet"}""").map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
  ...
  it should "delete a document set" in {
    apiClient.delete(uri = "http://localhost:8080/document_set2", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }
  ...
  it should "create a document with id and null content" in {
    apiClient.post(uri = "http://localhost:8080/document_set/hello_world", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
  ...
  it should "create a document with id and content" in {
    apiClient.post(uri = "http://localhost:8080/document_set/hello_world2", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
  ...
  it should "replace a document with content" in {
    apiClient.put(uri = "http://localhost:8080/document_set/hello_world", headers = List(Cookie(cookies)), entity = """{"name":"document"}""").map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
  ...
  it should "patch a document" in {
    apiClient.patch(uri = "http://localhost:8080/document_set/hello_world", headers = List(Cookie(cookies)), entity = """[{"op":"add","path":"/hello","value":"world"}]""").map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
        val result = jv.asJsObject.fields("result").asJsObject
        assert("hello_world" == result.fields("id").asInstanceOf[JsString].value)
  ...
  it should "create a search template" in {
    ...
    apiClient.post(uri = "http://localhost:8080/document_set/search001", headers = List(Cookie(cookies)), entity = template).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
        val result = jv.asJsObject.fields("result").asJsObject
  ...
  it should "search documents using search template" in {
    apiClient.get(uri = "http://localhost:8080/document_set/search001/_exec", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }
  ...
  it should "delete a document" in {
    apiClient.delete(uri = "http://localhost:8080/document_set/hello_world2", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }
  ...
  it should "delete a document set which have some documents" in {
    apiClient.delete(uri = "http://localhost:8080/document_set", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }
  ...
  it should "login in www.hello.com" in {
    ...
    apiClient.post(uri = "http://www.hello.com:8080/_login", entity = user).map {
      case (code, headers, jv) =>
        helloCookies = headers.filter(_.isInstanceOf[SetCookie]).map { v =>
          val cookie = v.asInstanceOf[SetCookie].cookie()
          HttpCookiePair(cookie.name(), cookie.value())
        }
        assert(code == StatusCodes.OK)
  ...
  it should "create a document set in www.hello.com" in {
    apiClient.post(uri = "http://www.hello.com:8080/document_set", headers = List(Cookie(helloCookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
        assert("document_set" == jv.asJsObject.fields("result").asJsObject.fields("id").asInstanceOf[JsString].value)
    }
  ...
  it should "create a document in www.hello.com" in {
    apiClient.post(uri = "http://www.hello.com:8080/document_set/hello_world", headers = List(Cookie(helloCookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.Created)
        assert("hello_world" == jv.asJsObject.fields("result").asJsObject.fields("id").asInstanceOf[JsString].value)
    }
  ...
  it should "delete a document in www.hello.com" in {
    apiClient.delete(uri = "http://www.hello.com:8080/document_set/hello_world", headers = List(Cookie(helloCookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }
  ...
  it should "reset the platform" in {
    apiClient.delete(uri = "http://localhost:8080/_reset", headers = List(Cookie(cookies))).map {
      case (code, _, jv) =>
        assert(code == StatusCodes.OK)
    }
  }
