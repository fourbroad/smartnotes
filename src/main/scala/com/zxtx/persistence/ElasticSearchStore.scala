...
object ElasticSearchStore {
  ...
  private val emptyEntity = HttpEntity.empty(ContentTypes.`application/json`)
...
class ElasticSearchStore(system: ActorSystem) {
  ...
  def newIndex(name: String): Future[(StatusCode, JsValue)] = put(uri = s"${host}/${name}", entity = emptyEntity)
  ...
  def deleteByQuery(index: String, parameterSeq: Seq[(String, String)], entity: RequestEntity): Future[(StatusCode, JsValue)] = {
    ...
    post(s"${host}/${index}/_delete_by_query?${params}", entity)
  }
  ...
  def request(method: HttpMethod = HttpMethods.GET, uri: Uri, entity: RequestEntity = emptyEntity): Future[(StatusCode, JsValue)] = {
