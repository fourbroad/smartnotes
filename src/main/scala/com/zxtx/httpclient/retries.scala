...
object SleepFuture {
  def apply[T](delay: FiniteDuration)(todo: => T)(implicit system: ActorSystem, ec: ExecutionContext): Future[T] = {
...
trait CountingRetry {
  ...
  protected def retry[T](
    ...
    promise: () => Future[T],
    success: Success[T],
    orElse: Int => Future[T])(implicit ec: ExecutionContext) = {
    val fut = promise()
    fut.flatMap { res =>
...
object Directly extends CountingRetry {
  def apply[T](max: Int = 5)(promise: () => Future[T])(implicit success: Success[T], ec: ExecutionContext): Future[T] = {
...
object Pause extends CountingRetry {
  def apply[T](max: Int = 5, delay: FiniteDuration = 500.milliseconds)(promise: () => Future[T])(implicit success: Success[T], system: ActorSystem, ec: ExecutionContext): Future[T] = {
    retry(
      max,
      promise,
      success,
...
object Backoff extends CountingRetry {
  def apply[T](max: Int = 5, delay: FiniteDuration = 500.milliseconds, base: Int = 2)(promise: () => Future[T])(implicit success: Success[T], system: ActorSystem, ec: ExecutionContext): Future[T] = {
    retry(
      max,
      promise,
      success,
